 ...
 * Activity's onDestroy method. It is invalid to dispose the object while an
 * asynchronous operation is in progress. You can call
 * {@link #disposeWhenFinished()} to ensure that any in-progress operation
 * completes before the object is disposed. A note about threading: When using
 * this object from a background thread, you may call the blocking versions of
 * methods; when using from a UI thread, call only the asynchronous versions and
 * handle the results via callbacks. Also, notice that you can only call one
 * asynchronous operation at a time; attempting to start a second asynchronous
 * operation while the first one has not yet completed will result in an
...
public class IabHelper {
    ...
    // Do we need to dispose this object after an in-progress asynchronous
    // operation?
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
        ...
        mServiceConn = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                try {
                    ...
                    // getBuyIntentToReplaceSku which allows for subscription
                    // update
        ...
        List<ResolveInfo> intentServices = mContext.getPackageManager().queryIntentServices(
                serviceIntent, 0);
    ...
    /**
     * Disposes of object, releasing resources. If there is an in-progress async
     * operation, this method will queue the dispose to occur after the
        ...
        synchronized (mAsyncInProgressLock) {
            if (mAsyncInProgress) {
                ...
                try {
                    ...
                    // Should never be thrown, because we call dispose() only
                    // after checking that
     ...
     * this object's {@link #handleActivityResult} method to continue the
     * purchase flow. This method MUST be called from the UI thread of the
     * Activity.
     *
     ...
     * @param oldSkus A list of SKUs which the new SKU is replacing or null if
     *            there are none
    ...
    public void launchPurchaseFlow(Activity act, String sku, String itemType, List<String> oldSkus,
        ...
        try {
            ...
            if (oldSkus == null || oldSkus.isEmpty()) {
                ...
                buyIntentBundle = mService.getBuyIntent(3, mContext.getPackageName(), sku,
                        itemType,
                        extraData);
            } else {
     ...
     * call this method from your Activity's {@link Activity @onActivityResult}
     ...
     * use the non-blocking version {@link #queryInventoryAsync}.
     *
        ...
        synchronized (mAsyncInProgressLock) {
            ...
            if (mDisposeAfterAsync) {
                try {
                    ...
                    // Should not be thrown, because we reset mAsyncInProgress
                    // immediately before
                    // calling dispose().
                }
            }
    ...
    /**
     * Exception thrown when the requested operation cannot be started because
